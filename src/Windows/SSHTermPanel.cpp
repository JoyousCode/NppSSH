...
void SSHTermPanel::setSSHConnected(bool state) {
    ...
    // 同步更新输出框状态提示
    if (_hEditTerm && ::IsWindow(_hEditTerm)) {
        if (state) {
            //OnSSHConnected(this->_panelId);         //调用转发SSH连接设置当前面板连接资源
            //TODO 待优化连接成功后的命令执行
            //::SetWindowTextW(_hEditTerm, L"输出框状态提示SSH连接成功！/r/n可执行SSH命令...");
            //SYSTEMTIME st;
            //GetLocalTime(&st);
            ...
            SSH_ConnectionOnDisconn(this->_panelHwnd);        //调用转发断开连接释放当前面板连接资源

        }
        NppSSH_LogInfoAuto("setSSHConnected==========面板唯一索引======" + std::to_string(this->_panelSeqId));
        //MessageBoxW(g_nppData._nppHandle, (L"当前面板ID==" + std::to_wstring(this->_panelId)).c_str(), L"NppSSH", MB_OK | MB_TASKMODAL);
        //自动滚动到底部
        DWORD len = ::GetWindowTextLengthW(_hEditTerm);
        ::SendMessageW(_hEditTerm, EM_SETSEL, len, len);
        ::SendMessageW(_hEditTerm, EM_SCROLLCARET, 0, 0);
    }
    // 解锁
...
{
    //int newIconSize = ::SendMessage(g_nppData._nppHandle, NPPM_GETTOOLBARICONSIZE, 0, 0);
    ::MessageBoxW(g_nppData._nppHandle, L"触发工具栏尺寸更新", L"NppSSH调试", MB_OK | MB_ICONINFORMATION);
    if (_hBtnConnectSSH && IsWindow(_hBtnConnectSSH)) {
        ...
        EnableWindow(_hBtnConnectSSH, isEnabled);// 恢复原来的状态
    }
    if (_hBtnDisconnectSSH && IsWindow(_hBtnDisconnectSSH)) {
        ...
        EnableWindow(_hBtnDisconnectSSH, isEnabled);// 恢复原来的状态
    }
}
...
INT_PTR CALLBACK SSHTermPanel::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    switch (message) {
    case WM_SET_BTN_TRUE:
    {
        setSSHConnected(_isConnected);
        return TRUE;
    }
    case WM_SSHLOGIN_CONNECTION_MSG:
    {
        bool isErrMsg = (wParam == 1);   // 判断是否err错误消息
        //CloseSshWaitDialog(_hSshWaitDlg);
        if (isErrMsg)
        {
    ...
    case WM_SSHLOGIN_BTNMSG:
    {
        ...
        pInput->bTestMsg ? _testMsgHandle = true : _testMsgHandle = false;
        if (!pInput->bTestMsg) {
            if (_hBtnConnectSSH) ::EnableWindow(_hBtnConnectSSH, FALSE);
            if (_hBtnDisconnectSSH) ::EnableWindow(_hBtnDisconnectSSH, FALSE);
        }
        HWND hWaitDlg =CreateSshWaitDialog(_panelHwnd,_hLoginPanel, _testMsgHandle);
        if (hWaitDlg == nullptr) { return TRUE; }
        //_hSshWaitDlg = hWaitDlg;
        
        //UpdateSshWaitProgress(_hSshWaitDlg, 40, L"正在连接SSH服务器...", nullptr);

        bool ok = SSH_ConnectionHandle(_panelHwnd, hWaitDlg, pInput->szHost,
            pInput->szPort, pInput->szUser, pInput->szPass, pInput->szDir);
        if (ok) {
            std::wstring msg = L"SSH " + testmsg + L"进行中...";
    ...
    case WM_SIZE:
    {
        ...
        if (sizeType == SIZE_MINIMIZED || nClientW <= 0 || nClientH <= 0)
            break;
        SetProp(_hEditTerm, L"NppSSH_PanelW", (HANDLE)(LONG_PTR)nClientW);
        SetProp(_hEditTerm, L"NppSSH_PanelH", (HANDLE)(LONG_PTR)nClientH);
    ...
    case WM_TIMER:
    {
        if (wParam == TIMER_ID_RESIZE_PTY)
        {
            ...
            SendMessageW(_hEditTerm, WM_USER_RESIZE_PTY, 0, 0);
        }
        break;
    }
    // 处理按钮点击消息
    case WM_COMMAND:
    {
        ...
        if (cmd == IDC_BTN_CONNECT_SSH) {
            ...
            if (_hLoginPanel == nullptr) {
                ...
                if (_hBtnDisconnectSSH) ::EnableWindow(_hBtnDisconnectSSH, true);
            } // 创建失败需要手动释放
    ...
    case WM_NOTIFY:
    {
        ...
        if (pnmh->hwndFrom == this->_hEditTerm)
        {
            //NppSSH_LogInfoAuto("父转发Terminal通知 code:" + std::to_string(pnmh->code));
            SendMessageW(_hEditTerm, WM_NOTIFY, wParam, lParam);
        }
        return TRUE;
    }
    ...
    // 工具栏图标大小变化时更新按钮图标
    case NPPN_TOOLBARICONSETCHANGED: // (暂时废除)
    {
...
// 重写背景色
void SSHTermPanel::setBackgroundColor(COLORREF color)
{
    _bgColor = color;
...
// 重写前景文字色
void SSHTermPanel::setForegroundColor(COLORREF color)
{
    _fgColor = color;
    ::InvalidateRect(GetHwndSelf(), nullptr, TRUE);
}
...
void SSHTermPanel::createTopButtonBar() {
    ...
    if (_hBtnConnectSSH) {
        ...
        ::MessageBoxW(g_nppData._nppHandle, L"连接按钮创建失败", L"NppSSH错误", MB_OK | MB_ICONWARNING);
    }
...
void SSHTermPanel::SetBackgroundImage(const WCHAR* imgPath)
{
    // 释放旧图片
    if (_hBgImage)
    {
...
INT_PTR CALLBACK SSHTermPanel::SSH_LoginDlgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_DESTROY:
        ...
        if (pPanel)
        {
            ...
            HWND hEdit = pPanel->GethEditTermHandle();
...
void SSHTermPanel_InitRecreatePanel(SSHBasePanel* pNewPanel) {
    ...
    if (pCon) {
        ...
        pCon->display(true);
        //::SendMessage(g_nppData._nppHandle, NPPM_DMMSHOW, 0, reinterpret_cast<LPARAM>(pCon->getHSelf()));
        // 额外触发标签栏重绘（兜底）
        //::RedrawWindow(g_nppData._nppHandle, NULL, NULL, RDW_FRAME | RDW_INVALIDATE | RDW_UPDATENOW);
